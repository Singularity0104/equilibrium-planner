...
def parse_script_line(string, index):
    ...
    while param_match:
        if '.' in param_match.group(2):
            instance_id = param_match.group(2).split('.')[1]
        else:
            instance_id = param_match.group(2)
        params.append(ScriptObject(param_match.group(1), int(instance_id)))
        ...
        for line in f:
            ...
            if len(line) > 0 and not line.startswith('#'):
                try:
                    ...
                    print(f'script line generation failed: {line}, {e}')
                    if 'Unknown action' in str(e) or ('Wrong number of parameters' in str(e)
                                                      and line.startswith('[') and line.endswith(')')):
                        error_line = line
                    break
    return Script(script_lines), error_line
