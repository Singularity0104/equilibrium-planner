...
import os
import gc
from equilibrium_model.plw_trainer import PLWTrainer
...
def finetune_world_model(args, model_path, data_path, save_data_path, save_lora_dir, save_model_dir, cache_dir):
    if os.path.exists(save_data_path):
        ...
        for p in data_path:
            print(f'loading {p}...')
            with open(p, 'rb') as f:
                raw_data += pickle.load(f)['train']
        training_data = []
        for i in tqdm(range(len(raw_data))):
            item = raw_data[i]
            training_data += generate_world_model_text(item)
    ...
    if args.prompt_loss_weight is None:
      ...
      # Training
    llama_fine_tuning.train(resume_from_checkpoint=args.resume)
